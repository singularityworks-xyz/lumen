...
describe("jwks-encryption", () => {
  ...
  describe("encryptPrivateKey / decryptPrivateKey", () => {
    ...
    it("output contains salt(32) + iv(12) + encrypted data", async () => {
      ...
      // Salt (32) + IV (12) + minimum encrypted data (at least 1 byte) + auth tag (16)
      expect(buf.length).toBeGreaterThan(44);
    });

    it("handles JSON-formatted private keys", async () => {
      const plaintext = JSON.stringify({
        kty: "RSA",
        n: "0vx7agoebGcQSuuPiLJXZptN9nndrQmbXEps2aiAFbWhM78LhWx4cbbfAAtVT86zwu1RK7aPzz_",
        e: "AQAB",
        d: "X4cTteJY_gn4FYPsXB8rdXix5vwsg1FLN5E3EaG6RJoVH-HLLKD9M7dx5oo7GURknchnR",
      });
      const encrypted = await encryptPrivateKey(plaintext);
      const decrypted = await decryptPrivateKey(encrypted);
      expect(decrypted).toBe(plaintext);
  ...
  describe("error handling", () => {
    ...
    it("decrypt fails on invalid base64", async () => {
      await expect(decryptPrivateKey("not-valid-base64!!!")).rejects.toThrow(
        "Decryption failed"
      );
    });

    it("decrypt fails on empty input", async () => {
      await expect(decryptPrivateKey("")).rejects.toThrow("Decryption failed");
    });
    ...
    it("handles corrupted salt/iv portion gracefully", async () => {
      const plaintext = "valid-key";
      const encrypted = await encryptPrivateKey(plaintext);
      const buf = Buffer.from(encrypted, "base64");
      // Corrupt only the salt portion (first 32 bytes)
      const corrupted = new Uint8Array(buf);
      for (let i = 0; i < 10; i++) {
        corrupted[i] = 255 - corrupted[i]!;
      }
      const corruptedBase64 = Buffer.from(corrupted).toString("base64");
      // Should fail because the derived key will be different
      await expect(decryptPrivateKey(corruptedBase64)).rejects.toThrow(
        "Decryption failed"
      );
    });
  ...
  describe("isEncryptionConfigured", () => {
    ...
    it("returns false when JWKS_ENCRYPTION_KEY is empty string", () => {
    ...
    it("returns true for long complex key", () => {
      process.env.JWKS_ENCRYPTION_KEY =
        "a very long and complex key with many characters!@#$%^&*()";
      expect(isEncryptionConfigured()).toBe(true);
    });
  });

  describe("edge cases", () => {
    it("handles binary-like data in plaintext", async () => {
      const plaintext = Buffer.from([0x00, 0x01, 0xff, 0xfe]).toString(
        "binary"
      );
      const encrypted = await encryptPrivateKey(plaintext);
      const decrypted = await decryptPrivateKey(encrypted);
    ...
    it("handles very long encryption key", async () => {
      process.env.JWKS_ENCRYPTION_KEY = "x".repeat(1000);
    ...
    it("handles short encryption key", async () => {
      process.env.JWKS_ENCRYPTION_KEY = "x";
      const plaintext = "test-data";
      const encrypted = await encryptPrivateKey(plaintext);
      const decrypted = await decryptPrivateKey(encrypted);
      expect(decrypted).toBe(plaintext);
