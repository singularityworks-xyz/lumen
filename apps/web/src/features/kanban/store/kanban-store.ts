...
> = (set, get) => ({
  ...
  openCreateTaskModal: (columnId, boardId, position) => {
    ...
    // Check if a task modal already exists for this board
    const existingModalForBoard = existingModals.find(
      (m) => m.boardId === boardId
    );
    if (existingModalForBoard) {
      // Bring existing modal to front instead of creating a new one
    ...
    // Use provided position (button location) or fall back to board-relative position
    let modalX: number;
    let modalY: number;
    if (position) {
      modalX = position.x;
  ...
  openEditBoardModal: (boardId, position) => {
    ...
    // Check if an edit modal already exists for this board
    const existingModalForBoard = existingModals.find(
      (m) => m.boardId === boardId
    );
    if (existingModalForBoard) {
