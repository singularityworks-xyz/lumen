...
describe("task-slice", () => {
  ...
  describe("duplicateTask", () => {
    it("inserts after source task with (Copy) suffix", () => {
      const { boardId, columnIds } = addBoardToState(state);
      const colId = columnIds[0];
      if (!colId) {
        throw new Error("missing colId");
    ...
    it("resets progress to 0", () => {
      ...
      const duplicatedTask = state.tasks.byId[result];
      expect(duplicatedTask).toBeDefined();
      if (!duplicatedTask) {
        throw new Error("duplicated task missing");
      }
      expect(duplicatedTask.progress).toBe(0);
    });

    it("resets checklist completed to false", () => {
      const { boardId, columnIds } = addBoardToState(state);
      const colId = columnIds[0];
      if (!colId) {
        throw new Error("missing colId");
      ...
      if (!sourceTask) {
        throw new Error("source task missing");
      }
      ...
      const dupTask = state.tasks.byId[result];
      expect(dupTask).toBeDefined();
      if (!dupTask) {
        throw new Error("duplicated task missing");
      }
      const checklists = dupTask.checklists;
